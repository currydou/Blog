...
/**
 * 1.xml里面第一个作为放在中心的位置，其它放在四周
 * 2.getWidth(): View在設定好佈局後整個View的寬度。
 *      getMeasuredWidth(): 對View上的內容進行測量後得到的View內容佔據的寬度(一直在onLayout完成后才会被赋值)
 *
 * @author zhy
 */
public class ArcMenu extends ViewGroup implements OnClickListener {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            // mesure child
            getChildAt(i).measure(MeasureSpec.UNSPECIFIED, MeasureSpec.UNSPECIFIED);//？？用UNSPECIFIED？
        }
    ...
    private void layoutButton() {
        ...
        switch (mPosition) {
            ...
            case RIGHT_TOP:
                ...
                break;
            case RIGHT_BOTTOM:
                left = getMeasuredWidth() - width;//这里为什么两个getMeasuredWidth获取的值不一样
                top = getMeasuredHeight() - height;
                break;

        }
        Log.e(TAG, left + " , " + top + " , " + (left + width) + " , " + (top + height));
        cButton.layout(left, top, left + width, top + height);
