...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    public GestureLockViewGroup(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPaint.setStrokeCap(Paint.Cap.ROUND);//圆形的笔触
        mPaint.setStrokeJoin(Paint.Join.ROUND);//设置接合处的形态
    ...
    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {//.......这里大小的确定
        ...
        if (mGestureLockViews == null) {
            ...
            mGestureLockViewWidth = (int) (4 * mWidth * 1.0f / (5 * mCount + 1));//.。。。。。这里计算宽度没看懂。。。。类注释有分析
            ...
            for (int i = 0; i < mGestureLockViews.length; i++) {
                ...
                // 不是每行的第一个，则设置位置为前一个的右边。。。。。。。。。。。。。。。。。。。第一个的位置在哪里设置的？？？
                if (i % mCount != 0) {
               ...
               if (i == 0) {
                ...
                if (i == 1) {
                    mGestureLockViews[1].setBackgroundColor(Color.YELLOW);
                }*/
